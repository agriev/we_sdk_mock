...
import './notif-liked-comment-on-review.styl';
...
const NotifLikedCommentOnReview = ({
  className,
  event,
  personal,
  ...
  const { reviews, user, games, comments } = event;
  ...
  return (
    <EventContainer
      className={cn('notif-liked-comment-on-review', className)}
      header={
        <EventHeader user={user} avatarIcon={likeIcon} emoji={false} eventDate={event.created}>
          <FormattedMessage
            id={`feed.feed_action_favorite_comment_review${personal ? '_personal' : ''}`}
            values={{
              user: userLink,
              comment: commentLink,
              game: gameLink,
    ...
    >
      <Comment
        ...
        arrowContent={
          <div
            onClick={() => goToReviewCommentFunc({ review, comment })}
            className="notif-liked-comment-on-review__go_to_discussion"
            role="button"
            tabIndex={0}
          >
...
NotifLikedCommentOnReview.propTypes = componentPropertyTypes;
NotifLikedCommentOnReview.defaultProps = defaultProps;
...
export default hoc(NotifLikedCommentOnReview);
