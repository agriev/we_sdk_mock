...
const NotifAddedCommentOnCollection = ({
  className,
  event,
  currentUser,
  size,
  ...
  const { user, games, collections, comments, collection_feeds } = event;
  ...
  return (
    <EventContainer
      ...
      header={
        <EventHeader user={user} emoji={false} eventDate={event.created}>
          <FormattedMessage
            id={`feed.feed_action_added_comment_collection${personal ? '_personal' : ''}`}
            values={{
              user: userLink,
              comment: commentLink,
    ...
    >
      ...
      <CollectionCard
        ...
        kind="inline"
        className="notifications-collection__item"
...
NotifAddedCommentOnCollection.propTypes = componentPropertyTypes;
NotifAddedCommentOnCollection.defaultProps = defaultProps;
...
export default hoc(NotifAddedCommentOnCollection);
