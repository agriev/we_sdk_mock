...
class EditModalSelect extends React.Component {
  ...
  renderSuggestedItem = (item) => {
    const { onClickOnItem, type, disableSelected } = this.props;

    const isDisabled = disableSelected(item);
    ...
    return (
      <div
        ...
        className={cn('game-edit__select-items__modal__select__available-item', {
          ...
          'game-edit__select-items__modal__select__available-item_disabled': isDisabled,
        })}
        onClick={isDisabled ? undefined : onClickOnItem(item)}
        role="button"
        tabIndex={0}
      >
